...
impl Builder {
    ...
    pub fn init_thread(
        ...
        collector::init(set_boxed_recorder, client, config);
        Ok(())
    }
    ...
    pub async fn init_future(
        ...
        let config = self.build_config()?;
        collector::init_future(set_boxed_recorder, client, config).await
    }
    ...
    pub async fn init_future_mock(
        ...
        let config = self.build_config()?;
        collector::init_future(set_boxed_recorder, mock, config).await
    }
